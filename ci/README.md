# CI: the vgi-geocode worker integration suite

[`.github/workflows/ci.yml`](../.github/workflows/ci.yml) runs the unit tests
and this repo's sqllogictest suite (`test/sql/*.test`) against the vgi-geocode
...
1. **Install the worker** — `uv sync --frozen` into a venv. `geocode_worker.py`
   is a self-contained PEP 723 stdio worker the extension can spawn via
   `uv run geocode_worker.py`. Reverse-geocoding is fully offline — the
   `reverse_geocoder` GeoNames index and `timezonefinder` polygon data ship
   inside their PyPI wheels, so there is nothing to download.
...
4. **Run** — [`run-integration.sh`](run-integration.sh) stages the preprocessed
   tree, points `VGI_GEOCODE_WORKER` at `uv run geocode_worker.py`, warms the
...
VGI_GEOCODE_WORKER="uv run --python 3.13 geocode_worker.py" \
  ci/run-integration.sh
```

Or use the Makefile target `make test-sql`, which installs `haybarn-unittest`
as a uv tool and points the worker at `uv run --python 3.13 geocode_worker.py`.
