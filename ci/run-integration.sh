...
#
# Run this repo's sqllogictest suite (test/sql/*.test) against the vgi-geocode
...
# ATTACH (`${VGI_GEOCODE_WORKER}`):
#
#   subprocess : a bare stdio command (`uv run geocode_worker.py`) — the extension
...
#                        `uv run --python 3.13 <repo>/geocode_worker.py`.
# Optional:
...
WORKER_CMD="${WORKER_CMD:-uv run --python 3.13 $REPO/geocode_worker.py}"
...
# ---------------------------------------------------------------------------
# Per-transport: resolve VGI_GEOCODE_WORKER (the LOCATION) and, for the out-of-band
...
case "$TRANSPORT" in
  subprocess)
    export VGI_GEOCODE_WORKER="$WORKER_CMD"
    ;;
  ...
  http)
    ...
    PORT_FILE="$(mktemp -u "${TMPDIR:-/tmp}/geocode-port.XXXXXX")"
    LOG_FILE="${TMPDIR:-/tmp}/geocode-http-server.log"
    ...
    export VGI_GEOCODE_WORKER="http://127.0.0.1:$PORT"
    ;;

  unix)
    # Boot the worker bound to an AF_UNIX socket; poll for the socket file.
    SOCK="${TMPDIR:-/tmp}/geocode-$$.sock"
    rm -f "$SOCK"
    LOG_FILE="${TMPDIR:-/tmp}/geocode-unix-server.log"
    ...
    export VGI_GEOCODE_WORKER="unix://$SOCK"
    ;;
...
echo "Running suite (transport: $TRANSPORT, worker: $VGI_GEOCODE_WORKER) ..."
OUT_FILE="$(mktemp)"
set +e
